...
7. 查看 Run monitor 步骤和 Summary；首次运行会把当前列表中符合规则的通知合并发送一次。
...
- 已发现通知、首次/最后发现时间、来源、发布时间、相关度和日期原句。
...
相同 URL + 相同标题及相同结构化关键日期不会重复提醒。标题发生变化或报名/材料/面试日期变化会形成新版本；相同标题不同 URL，只有同一学校且非空正文指纹也相同时才合并为转载别名，否则作为独立通知。旧文章重新置顶只更新最后发现时间。查询参数中的跟踪标记和已知 VSB content.jsp 地址会规范化。为避免误合并，不能证明是同一通知时宁可保留。

初次启动会抓取配置列表最近几页，并保留 120 天内的疑似招生通知和明确 2027 通知。2027 旧通知仍可能有参考价值，会首次提醒一次；不会每小时重发。学院来源加 5 分，推免/直博关键词加 5 分，2027/接收推荐免试/直接攻读博士加 4 分，报名/申请等加 3 分，目标学科加 2 分，招生词加 1 分。指定学院的推免/直博通知优先为五星。
...
日报包括七个学院的状态、监控页数、当日新记录、高优先级、即将截止、失败 URL 和错误、距离结束天数、未确认邮件事务。缺失发布时间或详情读取失败会显示部分正常。成功检查学院仅统计至少一个学院核心页面成功，不能仅靠校级页面把学院标成成功。
...
不会绕过验证码、登录或安全机制。robots.txt 明确禁止时停止该站抓取；规则服务临时不可达时保守报错。404 表示未提供规则；某些 CMS 对不存在的 robots.txt 返回首页，机器人解析器会忽略非规则文本。每站有合理间隔、超时和有限指数退避重试，403/404 不无限重试。分页最多 3 页，若列表发文量突然大增，应增大配置并观察运行耗时。
